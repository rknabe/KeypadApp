...
public final class DeviceManager implements HidServicesListener {
    ...
    public void hidDeviceAttached(HidServicesEvent hidServicesEvent) {
        ...
        if (hidDevice.getVendorId() == Device.LEONARDO_VENDOR_ID && hidDevice.getProductId() == Device.LEONARDO_PRODUCT_ID) {
            boolean open = hidDevice.open();
            if (open) {
                sleep(10);
                byte[] reportData = new byte[64];
                int r = hidDevice.read(reportData);
                for (int i = 0; i < 500; i++) {
                    int size = hidDevice.getFeatureReport(reportData, (byte) 3);
                    //r = hidDevice.sendFeatureReport(reportData, (byte) 3);
                    r = hidDevice.write(reportData, 64, (byte) 3);
                    if (size > 0) {
                        //byte[] data = hidDevice.readAll(11);
                        if (reportData.length > 0) {
                            boolean handled = handleInputReport(hidDevice, reportData);
                            if (handled) {
                                break;
                            }
                        }
    ...
    private SerialPort findMatchingCommPort(short uniqueId) {
        ...
        for (SerialPort port : ports) {
            if (port.getVendorID() == Device.LEONARDO_VENDOR_ID && port.getProductID() == Device.LEONARDO_PRODUCT_ID) {
